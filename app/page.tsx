  ...
  return (
    <main className="min-h-screen bg-[#fff6ec] text-slate-950">
      <section className="mx-auto max-w-7xl px-5 py-6 md:px-8">
        <header className="mb-5 flex flex-col gap-4 rounded-[2rem] border-4 border-slate-900 bg-white p-6 shadow-[8px_8px_0px_#111827] md:flex-row md:items-center md:justify-between">
          <div>
        ...
        <section className="mt-6 rounded-[2rem] border-4 border-slate-900 bg-white p-6 shadow-[8px_8px_0px_#111827]">
          ...
          <h2 className="mt-2 text-3xl font-black">
            {mode === "direct"
              ? "Why is this direct proportion?"
              : "Why is this inverse proportion?"}
          </h2>
          ...
          {mode === "direct" ? (
            <div className="mt-5 grid gap-4 md:grid-cols-3">
              <ExplanationTile
                title="One apple"
                text={`One apple costs Rs. ${pricePerApple}.`}
                emoji="🍎"
              />
              <ExplanationTile
                title={`${apples} apples`}
                text={`${apples} apples cost ${apples} × ${pricePerApple} = Rs. ${currentY}.`}
                emoji="🧺"
              />
              <ExplanationTile
                title="Pattern"
                text="If the number of apples doubles, the total cost also doubles."
                emoji="📈"
              />
            </div>
          ) : (
            <div className="mt-5 grid gap-4 md:grid-cols-3">
              <ExplanationTile
                title="Fixed work"
                text={`The total work is ${totalWork} worker-days.`}
                emoji="🧱"
              />
              <ExplanationTile
                title={`${workers} workers`}
                text={`${workers} workers finish the job in ${currentY.toFixed(
                  2
                )} days.`}
                emoji="👷"
              />
              <ExplanationTile
                title="Pattern"
                text="If the number of workers doubles, the days become half."
                emoji="📉"
              />
            </div>
          )}
...
function ExplanationTile({
  ...
  return (
    <div className="rounded-[1.5rem] border-4 border-slate-900 bg-[#fff6ec] p-5 shadow-[5px_5px_0px_#111827]">
